...
class InternalEnforcer(CoreEnforcer):
    ...
    def _add_policy(self, sec, ptype, rule):
        ...
        if not self.model.add_policy(sec, ptype, rule):
            return False
    ...
    def _add_policies(self,sec,ptype,rules):
        ...
        if not self.model.add_policies(sec, ptype, rules):
            return False
    ...
    def _update_policy(self, sec, ptype, old_rule, new_rule):
        ...
        if not self.model.update_policy(sec, ptype, old_rule, new_rule):
            return False
    ...
    def _remove_policy(self, sec, ptype, rule):
        ...
        if not self.model.remove_policy(sec, ptype, rule):
            return False
    ...
    def _remove_filtered_policy(self, sec, ptype, field_index, *field_values):
        ...
        if not self.model.remove_filtered_policy(sec, ptype, field_index, *field_values):
            return False
